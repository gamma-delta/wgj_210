...
pub struct ModePlaying {
    ...
    /// Position of the upper-left of the camera view on the board,
    /// where 1.0 = one grid square (6 pixels)
    view: Vec2,
    /// If this is Some, we're right-clicking to drag the camera view,
    /// and this contains the original mouse position.
    ///
...
impl ModePlaying {
    pub fn new_temp() -> Self {
        Self {
            ...
            view: -vec2(WIDTH, HEIGHT) / 2.0 / SYMBOL_GAP,
            drag_origin: None,
...
impl Gamemode for ModePlaying {
    fn update(
        ...
        let hovered_coord = ICoord::new(mouse_world.x.round() as _, mouse_world.y.round() as _);
